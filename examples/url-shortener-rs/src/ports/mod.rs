...
pub use clock::Clock;
pub use code_store::{Bind, CodeStore, Row};
pub use shortener::{Console, Shortened, Shortener};
...
/// The rest of the same hand-over.
///
/// Note what is **not** in this list: `Ttl` and `CodeWidth`. An adapter must
/// never see a policy value. The boundary is held by the type system here,
/// not by care.
pub use crate::domain::{LongUrl, Mapping, ShortCode, ShortenError, Timestamp};
