...
///
/// `Bash` is a write tool: `echo x > f` is an edit with a different spelling.
/// `Task` and `Monitor` are on the list because a read-only agent that can
/// start a helper agent, or reach a shell another way, is not read-only —
/// asked to write a file under these restrictions, the agent named both routes
/// itself. This is the set of ways to reach a write, not the set of tools whose
/// names contain "write".
...
impl Access {
    ...
    /// Passed as two arguments it made `claude -p` read the prompt's own words
    /// as tool names and die with "Input must be provided". Every build would
    /// have failed, and no test on the flag list would have noticed.
...
async fn ask<T: serde::de::DeserializeOwned>(
    ...
    let frontier = read_answer::<T>(Ok(claude_run_retry(prompt, cwd, timeout_secs, attempts, access).await));
...
async fn calibrate(repo_root: &Path) -> Calibration {
    ...
    let (answer, who) = ask::<FindingsEnvelope>(&prompt, PROBE_CODE, false, repo_root, 600, 1, Access::ReadOnly).await;
...
pub async fn run_review_with(target: &str, gate: &str, repo_root: &Path, reporter: Reporter) -> ReviewReport {
    ...
    for lens in LENSES {
        ...
        hunts.push((
            lens.key,
            tokio::spawn(async move { ask::<FindingsEnvelope>(&prompt, &code, truncated, &root, 600, DEFAULT_RETRIES, Access::ReadOnly).await }),
        ));
    }
    ...
    for f in candidates {
        ...
        checks.push((
            f,
            tokio::spawn(async move { ask::<VerdictEnvelope>(&prompt, &code, truncated, &root, 600, DEFAULT_RETRIES, Access::ReadOnly).await }),
        ));
    }
    for (f, c) in checks {
    ...
    for f in &report.confirmed {
        ...
        match claude_run_retry(&prompt, repo_root, 900, DEFAULT_RETRIES, Access::Write).await {
            Ok(reply) => {
...
///
/// A killed run left a `Cargo.toml` behind and the next run inherited it
...
/// and reported failure with nothing else. A build here was reported failed
/// while the code was correct: the gate *string* had been mangled before it
/// reached hexa, and `sh` was saying so on stderr the whole time. One line of
/// that output would have answered it instantly.
...
pub async fn run_build_with(
    ...
    for prio in DESIGN_PRIORITIES.iter().take(n) {
        ...
        tasks.push(tokio::spawn(async move {
            claude_run_retry(&prompt, &root, timeout_secs, retries, Access::ReadOnly).await
        }));
    }
    ...
    if let Err(e) = claude_run_retry(&build_prompt, repo_root, timeout_secs.saturating_mul(4), retries, Access::Write).await {
        build.note(format!("build agent error: {e}"));
        report.notes.push(format!("build agent error: {e}"));
    }
    build.note(format!("gate: {gate}"));
    let (ok, gate_output) = crate::direct_exec::run_evidence(gate, repo_root).await;
